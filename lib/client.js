...
class RancherClient {
    ...
    finishUpgradeService(serviceId) {
        Joi.assert(serviceId, Joi.string().required(), 'Must specify service id');
        return this._request('post', `/v2-beta/projects/${this.environmentId}/services/${serviceId}/?action=finishupgrade`);
    }

    upgradeService(serviceId, payload) {
        Joi.assert(serviceId, Joi.string().required(), 'Must specify service id');
        return this._request('post', `/v2-beta/projects/${this.environmentId}/services/${serviceId}/?action=upgrade`, { payload: JSON.stringify(payload)});
    }
